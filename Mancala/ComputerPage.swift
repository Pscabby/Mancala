...
struct ComputerPage: View {
                                    ...
                                    .onTapGesture {
                                        if turn1{
                                            ...
                                            DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                                                var index = ChoseHole(player2: player2)
                                                Player2Tap(player1: &player1, player2: &player2, score1: &score1, score2: &score2, data: data, turn1: &turn1, turn2: &turn2, index: &index)
                                            }
    ...
    func ChoseHole(player2:[HoleData])->Int{
        var index = (0...5).randomElement()!
        print(index)
        while(player2[index].amount==0){
            index = (0...5).randomElement()!
        }
        print(index)
        return index
    }
    ...
    func Player2Tap(player1:inout [HoleData],player2:inout [HoleData],score1: inout HoleData,score2: inout HoleData,data:HoleData,turn1:inout Bool,turn2:inout Bool,index:inout Int){
        let index1 = player1.firstIndex {
            $0.id == data.id
        }
        guard index1 != nil else {return}
        index = (index1 ?? 0)! + index
            let step = data.amount
            ...
            if own > 0{
                for i in 1...own{
                    player2[index - i].amount+=1
                    player2[index - i].items.append(item[ItemIndex])
                    ItemIndex+=1
                             LastStep = index - i
                }
            }
        ...
        player2[index].amount=0
            player2[index].items.removeAll()
            if LastStep == 20{
                turn2=true
                turn1=false
            }
                ...
                if LastStep > -1 && LastStep  < 6{
                    if player2[LastStep].amount==1{
                        score2.amount+=player1[LastStep].amount
                        score2.amount+=1
                        player1[LastStep].amount=0
                        player2[LastStep].amount=0
                        player1[LastStep].items.removeAll()
                        player2[LastStep].items.removeAll()
                    }
